...
/**
 * Навигатор по аналогии с [androidx.navigation.fragment.FragmentNavigator]
 * Однако вместо того чотбы заменять фрагменты, он дбавляент новые поверх
 *
 * Это позволяет не переинициализировать фрагменты каждый раз, а рисовать новые поверх предыдущих.
 * визуально ускоряет приложение, хотя и ест чуть больше памяти
 *
 */
@Navigator.Name("addfragment")
open class AddFragmentNavigator(
    ...
    override fun popBackStack(popUpTo: NavBackStackEntry, savedState: Boolean) {
        ...
        if (savedState) {
            ...
            if(fragmentManager.fragments.size > 1){
                fragmentManager.fragments[fragmentManager.fragments.size-2].let {
                    if(it is ManualResumeListener){
                        it.onManualResume()
                    }
                    it.childFragmentManager.fragments.forEach { child ->
                        if(child is ManualResumeListener){
                            child.onManualResume()
                        }
                    }
                }
            }
